...
class AppName {
    ...
    static get powerpoint() { return 'powerpoint'; }
    static get excel() { return 'excel'; }
    static get visio() { return 'visio'; }
    static get access() { return 'access'; }
    static get project() { return 'project'; }
    static get publisher() { return 'publisher'; }
    static get spd() { return 'spd'; }
    static get pdf() { return 'pdf'; }
    static get fbx() { return 'fbx'; }
    static get dae() { return 'dae'; }
    static get obj() { return 'obj'; }
    static get infopath() { return 'infopath'; }
    static get sharecad() { return 'sharecad'; }
...
class AppConstants {
    static get userAgentAndroid() { return 'android'; }
    static get userAgentIphone() { return 'iphone'; }
    static get userAgentIpad() { return 'ipad'; }
    static get userAgentCompasAR() { return 'compasar'; }
    static get userAgentUWP() { return 'uwp'; }
    static get userAgentWindows() { return 'windows'; }
}
...
class IconFiles {
    static get fbx() { return 'assets/icons/2689-512.png'; }
    static get dae() { return 'assets/icons/2683-512.png'; }
    static get sharecad() { return 'assets/icons/411714-200.png'; }
    static get general() { return 'assets/icons/general.png'; }
}
...
class IconFonts {
    static get word() { return 'file-icon fa fa-file-word-o'; }
    static get pdf() { return 'file-icon fa fa-file-pdf-o'; }
    static get image() { return 'file-icon fa fa-file-image-o'; }
    static get excel() { return 'file-icon fa fa-file-excel-o'; }
    static get powerpoint() { return 'file-icon fa fa-file-powerpoint-o'; }
    static get commonFile() { return 'file-icon fa fa-file'; }
    static get text() { return 'file-icon fa fa-file-text-o'; }
}
...
class ItemTypes {
    ...
    static get qrcode() { return 'qrcode'; }
    static get revit() { return 'revit'; }
    static get pmo() { return 'pmo'; }
    static get map() { return 'map'; }
    static get lineItem() { return 'lineItem'; }
    static get fbxmodels() { return 'fbxmodels'; }
    static get submittal() { return 'submittal'; }
    static get installationOnm() { return 'installationOnm'; }
    static get approvalDnR() { return 'approvalDnR'; }
    static get lineItemRevit() { return 'lineItem-revit'; }
    static get file() { return 'file'; }
    static get folder() { return 'folder'; }
    static get ONM() { return 'ONM'; }
    static get DWG() { return 'DWG'; }
    static get fbx() { return 'fbx'; }
    static get planDrawings() { return 'planDrawings'; }
    static get lineItemplanDrawings() { return 'lineItem-planDrawings'; }
}
...
class UserRole {
    static get pmoAllowedRole() { return ['BU17_Proj_Mgmt_Office']; }
    static get pmoAllowedId() { return ['KRISNX']; }
}
...
let C3Service = class C3Service {
    constructor(http, staticService) {
        this.http = http;
    ...
    envSettings() {
        this.staticService.getEnvironmentSettings().subscribe(data => {
            if (data.environment === 'production') {
                this.rootFolderId = data.production.rootFolderId;
                this.rootAppName = data.production.rootAppName;
                this.rootPathName = data.production.rootPathName;
            }
            else {
                this.rootFolderId = data.quality.rootFolderId;
                this.rootAppName = data.quality.rootAppName;
                this.rootPathName = data.quality.rootPathName;
            }
        });
    }
    getPssDocument() {
    ...
    getHomeList(fileName, NotAllowedFolders) {
        this.envSettings();
        const formData = {
            'applicationType': '200',
            'attributeRequired': false
        ...
        return this.http.post(this.c3ServicesFolderFiles, formData, httpOptions).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_5__["map"])(list => {
    ...
    getFileVersions(title, path) {
        let payload;
        const intiFolder = path.split('/')[0];
    ...
    uploadFile(folderPath, file, fileType, comment) {
        this.envSettings();
        let payload = '';
        const intiFolder = folderPath.split('/').reverse()[0];
        payload = `{
      ...
      'comment': '${comment}'
        ...
        const req = new _angular_common_http__WEBPACK_IMPORTED_MODULE_2__["HttpRequest"]('POST', this.uploadUrl, fd, {
            headers: headers,
            reportProgress: true
        });
        return this.http.request(req).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_5__["map"])(event => this.getEventMessage(event, file)));
    }
...
let FileManageService = class FileManageService {
    constructor() { }
    getFileIcon(fileNameUrl) {
        // return font awesome icon class based on file
        let fileIcon = _constants__WEBPACK_IMPORTED_MODULE_2__["IconFonts"].commonFile;
        const fileName = fileNameUrl.toString().split('/').reverse()[0];
        const extension = fileName.includes('|a') ?
            fileName.substring(0, fileName.indexOf('|a')).split('.').reverse()[0] : fileName.split('.').reverse()[0];
        _constants__WEBPACK_IMPORTED_MODULE_2__["FileTypes"].typeExtensions.forEach(file => {
    ...
    getViewer(url) {
        const fileName = url.toString().split('/').reverse()[0];
        const extension = fileName.split('.').reverse()[0];
        let viewertype;
        _constants__WEBPACK_IMPORTED_MODULE_2__["FileTypes"].typeExtensions.forEach(file => {
    ...
    getFileType(url) {
        const fileName = url.toString().split('/').reverse()[0];
        const extension = fileName.split('.').reverse()[0];
        let typeName = `${extension.toUpperCase()} File`;
        _constants__WEBPACK_IMPORTED_MODULE_2__["FileTypes"].typeExtensions.forEach(file => {
...
/*!************************************!*\
  !*** ./src/app/_services/index.ts ***!
  \************************************/
/*! exports provided: C3Service, StaticDataService, FileManageService, UserService */
/***/ (function(module, __webpack_exports__, __webpack_require__) {
...
let ShareFileService = class ShareFileService {
    ...
    addProgressValue(fileUrl, value) {
        this.filFolderTree.map(item => {
    ...
    uploadComplete(fileUrl, c3Location) {
        this.filFolderTree.map(item => {
            if (item.text === fileUrl) {
...
let StaticDataService = class StaticDataService {
    ...
    getHomeStatic(NotAllowedFolders) {
        let headers = new _angular_common_http__WEBPACK_IMPORTED_MODULE_2__["HttpHeaders"]();
        headers = headers.set('errorMessage', 'false').set('loading', 'false');
        return this.http.get(this.homeStaticUrl, { headers }).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_3__["map"])(list => {
    ...
    getStaticPermissionData() {
        let headers = new _angular_common_http__WEBPACK_IMPORTED_MODULE_2__["HttpHeaders"]();
        headers = headers.set('errorMessage', 'false').set('loading', 'false');
        return this.http.get(this.userRoleStaticUrl, { headers });
    }
    callForEnvironmentData() {
        let headers = new _angular_common_http__WEBPACK_IMPORTED_MODULE_2__["HttpHeaders"]();
        headers = headers.set('errorMessage', 'false').set('loading', 'false');
        const promise = this.http.get(this.envSettingsUrl, { headers })
            .toPromise()
            .then(settings => {
            this.envName = new rxjs__WEBPACK_IMPORTED_MODULE_4__["BehaviorSubject"](settings);
            return settings;
        });
        return promise;
    }
...
let UserService = class UserService {
    ...
    checkRole() {
        ...
        return this.http.post(this.searchUrl, payload, { headers });
    }
};
...
function init_app(staticDataService) {
    return () => staticDataService.callForEnvironmentData();
}
...
let AppLoadService = class AppLoadService {
    constructor(httpClient, staticDataService) {
        this.httpClient = httpClient;
        this.staticDataService = staticDataService;
    }
    getSettings() {
        const promise = this.httpClient.get('./assets/json/misc-settings.json')
            .toPromise()
            .then(settings => {
            return settings;
        });
...
let AddFromCompasDriveDialogComponent = class AddFromCompasDriveDialogComponent {
    constructor(driveService, dialog, userService, snackBar, staticData, fileManage, dialogRef) {
    ...
    ngOnInit() {
        this.staticData.callForEnvironmentData().then(data => {
            this.staticData.getEnvironmentSettings().subscribe(envData => {
                if (envData.environment === 'production') {
                    this.rootFolderId = envData.production.rootFolderId;
                }
                else {
                    this.rootFolderId = envData.quality.rootFolderId;
                }
    ...
    initialService() {
        ...
        this.loading = true;
        this.userService.checkRole().subscribe(user => {
            if (user.userDetails[0]) {
                this.staticData.getStaticPermissionData().subscribe(permission => {
                    let [pmo, qrcode, revit, map, global] = [0, 0, 0, 0, 0];
                    user.userDetails[0].roleIdList.forEach(role => {
                    ...
                    this.NotAllowedFolders = [];
                    if (pmo === 0) {
                        this.NotAllowedFolders.push('pmo');
                        this.NotAllowedFolders.push('PMO');
                    }
                    if (qrcode === 0) {
                        this.NotAllowedFolders.push('qrcode');
                    }
                    if (revit === 0) {
                        this.NotAllowedFolders.push('revit');
                    }
                    if (global === 0) {
                        this.NotAllowedFolders.push('global');
                    }
                    if (map === 0) {
                        this.NotAllowedFolders.push('map');
                        this.NotAllowedFolders.push('MAP');
                        this.NotAllowedFolders.push('Map');
                    }
                    this.getInitialList();
    ...
    updateTable() {
        this.tableData = [...this.PMOStaticList.folders, ...this.PMOStaticList.files];
    }
    getInitialList() {
        this.driveService.getHomeList(this.qtNumber, this.NotAllowedFolders).subscribe(data => {
            if (data.filesCount <= 0 && data.foldersCount <= 0) {
                this.snackBar.open('No documents available in COMPAS drive', 'Close', {
                    duration: 2000,
                });
                this.loading = false;
                // this.dialogRef.close();
                ...
                if (this.qtNumber != null) {
                    ...
                    if (this.breadCrumb.length === 0) {
                        const bCrumb = QtFolder;
                        this.breadCrumb.push(bCrumb);
                        this.currentFolder = QtFolder;
                    }
                }
                this.staticData.getHomeStatic(this.NotAllowedFolders).subscribe(item => {
                    if (data.foldersCount > 0) {
                        const nameArray = data.folders.reduce((acc, curr) => {
                            acc.push(curr.name);
                            return acc;
                        }, []);
                        item.forEach(el => {
                            if (!nameArray.includes(el.name)) {
                                this.PMOStaticList.folders.push(el);
                            }
                        ...
                        this.PMOStaticList.foldersCount = 3;
                        this.PMOStaticList.folders = item;
                    }
                    this.updateTable();
                });
                ...
                const folders = this.PMOStaticList.folders;
        ...
        error => {
            if (error.error.message.includes('Could not find any folder with name')) {
                this.createFolderWithQuoteNumber();
            }
            else {
                this.loading = false;
                this.snackBar.open('Not able to connect to Compas Drive. Please try again later or contact Compas Customer support', 'Close', {
    ...
    createFolderWithQuoteNumber() {
        this.driveService
            .createFolder(this.qtNumber, this.rootFolderId).subscribe(() => {
            this.getInitialList();
        });
    }
    getCurrentPath() {
    ...
    onSelectionChange(folder) {
        ...
        if (this.currentFolder.type === _constants__WEBPACK_IMPORTED_MODULE_4__["ItemTypes"].global) {
            this.driveService.getHomeList('Global', this.NotAllowedFolders).subscribe(data => {
                this.PMOStaticList = data;
                const path = this.getCurrentPath();
                if (this.PMOStaticList.files.length > 0) {
                    this.PMOStaticList.files.map(file => {
    ...
    loadSecondaryList(parentId, parentFolderType) {
        if (parentFolderType === _constants__WEBPACK_IMPORTED_MODULE_4__["ItemTypes"].pmo) {
            this.staticData.getPmoStatic().subscribe(item => {
                this.PMOStaticList.folders = item;
                const folders = this.PMOStaticList.folders;
                const breadCrumb = this.breadCrumb;
                this.updateTable();
            ...
            this.driveService.getChilds(parentId).subscribe(data => {
                this.PMOStaticList = data;
                if (this.PMOStaticList.filesCount > 0) {
                    const path = this.getCurrentPath();
                    // fileData.fileEntryId = this.fileManage.getFileType(fileData.url.toString()),
                    this.PMOStaticList.files.map(fileData => {
                        fileData.sendUrl = fileData.url.toString();
                        fileData.typeName = this.fileManage.getFileType(fileData.url.toString()),
                            fileData.iconClass = this.fileManage.getFileIcon(fileData.url.toString());
                ...
                if (this.currentFolder.name.toUpperCase() === 'PMO') {
                    this.staticData.getPmoStatic().subscribe(item => {
                        const nameArray = data.folders.reduce((acc, curr) => {
                            acc.push(curr.name);
                            return acc;
                        ...
                        item.forEach(pmo => {
                            if (!nameArray.includes(pmo.name)) {
                                this.PMOStaticList.folders.push(pmo);
                            }
                        });
                        const folders = this.PMOStaticList.folders;
                        const breadCrumb = this.breadCrumb;
                ...
                if (this.PMOStaticList.folders.length > 0) {
                    const folders = this.PMOStaticList.folders;
                    const breadCrumb = this.breadCrumb;
                }
                this.updateTable();
    ...
    filterSearch(key) {
        ...
        this.folderFiltered = this.tableData.filter(data => {
            return (data.type !== 'file' && data.name.toUpperCase().includes(key.toUpperCase()));
        });
        this.fileFiltered = this.tableData.filter(data => {
    ...
    documentList() {
        this.breadCrumb = [];
        this.driveService.getPssDocument().subscribe(data => {
            this.PMOStaticList = data;
            if (this.PMOStaticList.files.length > 0) {
                this.PMOStaticList.files.map(file => {
                    file.type = _constants__WEBPACK_IMPORTED_MODULE_4__["ItemTypes"].file;
...
AddFromCompasDriveDialogComponent.ctorParameters = () => [
...
let DataFileListComponent = class DataFileListComponent {
    // public checkAll: boolean;
    constructor(dialog, driveService, router, fileManager, snackBar) {
        this.dialog = dialog;
        this.driveService = driveService;
        this.router = router;
    ...
    getIconFile(url) {
        const icon = this.fileManager.getFileIconPath(url);
        return icon;
    }
    // isCheckableAll() {
    //   let status = false;
...
DataFileListComponent.ctorParameters = () => [
    { type: _angular_material__WEBPACK_IMPORTED_MODULE_2__["MatDialog"] },
    { type: _services__WEBPACK_IMPORTED_MODULE_3__["C3Service"] },
...
let PssConfigComponent = class PssConfigComponent {
    constructor(c3Services, dialog, shareFileService) {
    ...
    ngOnInit() {
        this.resetTotal();
        this.c3Services.getPssDocument().subscribe(data => {
            this.drawings = data;
            this.drawings.files = this.drawings.files.filter(item => { return item.title !== 'Proposal16102019.png'; });
            this.drawings.files.forEach(item => {
    ...
    updateTotal() {
        this.resetTotal();
        this.drawings.files.forEach(item => {
            this.total.mv_breaker += item.device.mv_breaker;
            this.total.mv_fuses += item.device.mv_fuses;
            this.total.mv_trans += item.device.mv_trans;
            this.total.mv_motor += item.device.mv_motor;
            this.total.mv_cabs += item.device.mv_cabs;
            this.total.lv_sw_fuse += item.device.lv_sw_fuse;
            this.total.lv_adj_brk += item.device.lv_adj_brk;
            this.total.lv_non_adj_brk += item.device.lv_non_adj_brk;
            this.total.lv_trans += item.device.lv_trans;
            this.total.lv_pan_100 += item.device.lv_pan_100;
            this.total.lv_pan_50 += item.device.lv_pan_50;
            this.total.lv_cp += item.device.lv_cp;
            this.total.genset += item.device.genset;
        });
    }
    resetTotal() {
        this.total = {
    ...
    openAddFromCompasDriveDialog() {
        // console.log('Add from compas drive called!!');
        const dialogRef = this.dialog.open(_add_from_compas_drive_dialog_add_from_compas_drive_dialog_component__WEBPACK_IMPORTED_MODULE_5__["AddFromCompasDriveDialogComponent"], {
            width: `500px`,
            data: {}
        });
    ...
    fileChange($event) {
        ...
        Array.from(this.filesFromDevice).forEach(file => {
            console.log(file);
            this.shareFileService.addFile(new ngx_treeview__WEBPACK_IMPORTED_MODULE_7__["TreeviewItem"]({ text: file.name, value: 1 }));
            console.log(file);
            this.c3Services.uploadFile(`Global/PSS Documents`, file, null, 'comment').subscribe(data => {
                // console.log('data inside filechange', data);
                // console.log('file inside filechange', file);
                if (typeof data === 'number') {
                    this.inProgress = true;
                    // console.log(data);
                    this.shareFileService.addProgressValue(file.name, data);
                    console.log(data);
                    if (this.drawings.files.filter(item => { return (item.title === file.name); }).length === 0) {
                        this.drawings.files.push({ title: file.name, url: '', version: '1', createDate: '', modifiedDate: '', device: new src_app_models__WEBPACK_IMPORTED_MODULE_3__["FileDevice"](), loading: data });
                    }
                    else {
                        this.drawings.files.filter(item => { return (item.title === file.name); })[0].loading = data;
                    }
                }
                    ...
                    this.drawings.files.filter(item => { return (item.title === file.name); })[0].url = data.c3Location;
                    // this.message = `
                    // ${this.message} <br>
                    // <a style = 'text-decoration: none;'
                    // href='${data.c3Location}' target='_self'>${data.c3Location.split('/').reverse()[0]}</a>
                    // `;
                    this.shareFileService.uploadComplete(file.name, data.c3Location);
                }
            });
        });
    }
    scanDocument() {
        ...
        this.c3Services.getProposal().subscribe(filedata => {
            const file = new File([filedata], "Proposal" + Date.now().toString() + ".png", { lastModified: 1534584790000 });
            ;
            this.c3Services.uploadFile(`Global/PSS Documents`, file, null, 'comment').subscribe(data => {
                // console.log('data inside filechange', data);
                // console.log('file inside filechange', file);
...
PssConfigComponent.ctorParameters = () => [
